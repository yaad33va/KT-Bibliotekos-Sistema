<x-layouts.app :title="__('Available Books')">
    <div class="space-y-6">
        <div>
            <h1 class="text-3xl font-bold">{{ __('Available Books') }}</h1>
        </div>

        @if($books->isEmpty())
            <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-8 text-center">
                <p class="text-gray-600 dark:text-gray-400">{{ __('No books available for borrowing.') }}</p>
            </div>
        @else
            <div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                @foreach($books as $book)
                    <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-4 space-y-3">
                        ...
                        <p class="text-sm font-medium">{{ __('Available') }}: {{ $book->available_copies }}/{{ $book->book_count }}</p>

                        @auth
                            @if(auth()->user()->hasRole('registered'))
                                <x-form method="post" :action="route('borrowing.borrow', $book)" class="pt-2">
                                    <x-button class="w-full">{{ __('Borrow Book') }}</x-button>
                                </x-form>
                            @endif
                        @endauth
                    </div>
